...
class ArticleWithCommentsResponseTest {
    ...
    void givenArticleWithCommentsDtoWithoutChildComments_whenMapping_thenOrganizesCommentsWithCertainOrder() {
        ...
        Set<ArticleCommentDto> articleCommentDtos = Set.of(createArticleCommentDto(1L, null, now),
    ...
    void givenArticleWithCommentsDto_whenMapping_thenOrganizesParentAndChildCommentsWithoutDepthLimit() {
        ...
        /*
           Iterator: 자바에서 컬렉션을 순회하는 데 사용되는 인터페이스, 컬렉션의 요소에 접근하고 해당 요소를 수정할 수 있는 표준화된 방법을 제공한다.
           hasNext():   다음 요소가 존재하는지 확인
           E next():    다음 요소를 반환한다.
           remove():    현재 next()로 반환된 요소를 삭제한다. 이 메서드는 옵션으로 제공되며, 일부 컬렉션에서는 지원되지 않을 수 있다.
                    */
