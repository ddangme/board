...
class UserAccountServiceTest {
    ...
    void givenExistentUserId_whenSearching_thenReturnsOptionalUserData () {
        ...
        // Then
        assertThat(result).isPresent(); // isPresent( ): Optional 객체가 값이 있는지 여부를 확인하는데 사용된다. 값이 존재하면 true, 없으면 false를 반환한다.
        then(userAccountRepository).should().findById(username);
    }

    @DisplayName("존재하지 않은 회원 ID를 검색하면, 비어있는 Optional로 반환한다.")
    @Test
